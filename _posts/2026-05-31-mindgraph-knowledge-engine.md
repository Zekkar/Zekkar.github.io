...
它就是一支 CLI：`mindgraph ingest` 把 `raw/` 編譯成 `wiki/`，`mindgraph search` 查詢，`mindgraph coverage` 量化「程式碼有多少被知識庫覆蓋」。MIT 授權。
...
> ⚙️ **v0.1.0 現況**：下面描述的 `two_stage_hybrid`（圖 + 向量合併）與第 7 節的 IntentRouter，都已經在程式碼裡實作好，但**尚未接上 CLI**——v0.1.0 的 `mindgraph search` 目前仍是純 TF-IDF 關鍵字搜尋。把這兩個 primitive 接到 search 是 roadmap Phase 3 的工作。以下講的是它們的設計與打算怎麼運作。
...
MindGraph 目前是 **v0.1.0 MVP**（MIT 授權，19 個 pytest）：

- **Phase 1（現在）**：CLI（ingest / embed / stats / search / check / coverage）、Provider 抽象、FileSystemAdapter + custom adapter 協定、WikiGraphEngine、EmbeddingStore（pgvector）、idempotent ingest + 原子 state
- **Phase 2（Server）**：REST API（`mindgraph serve`）、給 Claude Code 用的 MCP server
- **Phase 3（Scale）**：把 `two_stage_hybrid` 接到 REST search endpoint、IntentRouter 接上 search、file-diff 增量建圖、概念演化時間軸（devdiary 整合）。至於交易部署驗證過的時間衰減與主動注入，目前是我個人想回灌進開源核心的方向，README roadmap 尚未正式列入

部署用 Docker（`mindgraph` 引擎 + `pgvector/pgvector:pg16`）一鍵起。
...
*MindGraph 技術棧：PostgreSQL + pgvector、NetworkX、Python。LLM / Embedding 走可替換的 Provider 抽象（Anthropic / OpenAI / Gemini）。*
