...
它是一支 CLI 加一層 server：`mindgraph ingest` 把 `raw/` 編譯成 `wiki/`，`mindgraph search --hybrid` 做意圖感知檢索，`mindgraph serve` / `mindgraph mcp` 把知識庫以 REST 或 MCP 對外（給 Claude Code 當外部記憶），`mindgraph coverage` 量化「程式碼有多少被知識庫覆蓋」。MIT 授權。
...
- **Phase 2（✅ v0.2.0）**：REST API（`mindgraph serve`，10 端點，預設綁 127.0.0.1）、MCP server（`mindgraph mcp`，7 tools，給 Claude Code 當外部記憶）、Gemini 遷到 `google.genai` 新 SDK
- **Phase 3（部分完成）**：✅ `two_stage_hybrid` + IntentRouter 接上 `search --hybrid` 與 REST／MCP `smart_search`；⬜ file-diff 增量建圖、概念演化時間軸（devdiary 整合）。交易部署驗證過的時間衰減與主動注入，是我想回灌進開源核心的方向，尚未列入正式 roadmap
